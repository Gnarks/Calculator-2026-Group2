...
public class TestEvaluationController {
    ...
    @Test
    void testResponseWhenGenerateRandom() throws Exception {
        EvaluationResponse response = new EvaluationResponse(1, "122");
        when(evaluationService.getRandomNumber(AtomType.INTEGER, 150, null)).thenReturn(response);
        mockMvc.perform(get("/api/random?type=INTEGER&max=150"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.result").value("122"))
                .andExpect(jsonPath("$.success").value("1"));
    }
